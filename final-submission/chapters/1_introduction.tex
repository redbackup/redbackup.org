...
In this chapter, we describe previous work and existing applications in this area. To summarise, the goals of this report might be split into three categories:
...
Borg also has the ability to create backups to remote locations (e.g. over SSH), with a server-mode Borg instance running on the remote location. The downside of this implementation is that the \gls{client} still needs full write access to the backup server, and hence would permit malware to possibly delete backups.
...
\paragraph{Rsync,} \cite{rsync} originally designed for file synchronisation, is also often used to create backups today. With the help of filesystem-hardlinks it has file-deduplicating capabilities and can also sync files to remote locations, so that old backup cannot be modified.

The two most major downsides of rsync are that it must be combined with several other applications (e.g. ssh, bash scripts, ceph) to provide a secure backup solution and that it is relatively complicated to set up and configure.



\subsection{Peer-to-Peer Backup Storage}

There exist many approaches to create distributed, encrypted peer-to-peer storage systems. Two notable representatives, that create such a system decentralised over the internet, are Tahoe-LAFS \cite{tahoe-lafs} and IPFS \cite{ipfs}.
...
Another wide and well researched market are distributed filesystems, with prominent representatives as the Andrew File System~\cite{afs} and ceph~\cite{ceph}. These filesystems are commonly used in low latency, directly connected environments as data centers.
...
% Technology?
In our paper we focus mainly on a concrete backup software, introducing a practical solution that might be used by small enterprises and individuals. The architecture should provide developers with a clear guideline, how to implement such a system, with a prototype demonstrating a minimal implementation of such a system.

\subsection{Satisfaction}
% What goals were satisfied? How and why?
% Details are covered in Chapter "Discussion and Conclusion"
% Note that this is very similar to the management summary result section.
