...
The \emph{Client} knows the \emph{Management} by configuration.
...
\begin{itemize}
	...
	\item reply to \emph{Backup Requests}  (Scenario \ref{sec:scenario-create-backup} \nameref{sec:scenario-create-backup}) , \emph{client list root handles} (Scenario \ref{sec:scenario-backup-restore} \nameref{sec:scenario-backup-restore}) and \emph{fetch chunk}s requests
...
The main purpose of a \emph{Storage} component is to manage \emph{Chunk}s.
...
The \emph{Administrator} is the only Actor interacting with the \emph{Management}. He/She sends instructions (e.g. add new node) and is notified by the \emph{Management} about any anomalies.
   ...
   \item If the contact was established, the client starts creating a backup.
        \begin{enumerate}
            ...
            \item The designated node checks, if all chunks received from the client chunk list are present on the node.
                ...
                \end{itemize}
            \item The client sends the requested chunks to the designated node with a \emph{post chunks} message. %TODO: Abschluss client transaktion (root handle)
                \begin{enumerate}
            ...
            \item The client sends the additional chunks (as \emph{post chunks} messages) to the designated node, in which the \emph{root handle} is highlighted. %TODO: The root handle holds a reference to the first list chunk as well as owner etc. information.
        \end{enumerate}
\end{enumerate}
...
\begin{enumerate}
    ...
    \item The new node configures itself based on its \emph{node identifier} and the response from the management.
        ...
        \end{itemize}
    \item Other nodes learn about the new node through periodical metadata queries (message: \emph{post nodes metadata}) %TODO: Component description: regular queries
        \begin{enumerate}
