...
\paragraph{Responsibilities}
\begin{itemize}
    \item Instruct to the \gls{management} to add/remove \glspl{node} from the System
    \item Ensure that the \gls{management} runs properly (i.g. monitor it)
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Serialize the \gls{chunk-index}
...
\begin{itemize}
    ...
    \item A \gls{node} is in a given \gls{state} and acts differently depending on it (See Figure \ref{fig:node-states}).
    \begin{itemize}
        \item \emph{uninitialized}: The \gls{node} queries the \gls{management} for initialization and does nothing else.
        \item \emph{participating}: This is the "normal" state in which a \gls{node} accepts backups, performs replication and sends requested data.
        \item \emph{unreachable}: A \gls{node} is marked as \emph{unreachable} if any other \gls{node} or the \gls{management} can not reach it. A node never sees itself in this state.
        \item  \emph{leaving soon}: The \gls{node} does not answer any requests and ensures that all its data is replicated. When it is done, it automatically switches into the \emph{left} state.
        \item \emph{left}: The \gls{node} has nothing to do in this state and can shut down.
    \end{itemize}
    \item A \gls{node} knows all other \glspl{node} in the System and maintains a \gls{state} (see Figure~\ref{fig:node-states}) for them as well.
    \item The \glspl{chunk-content} are stored in a \gls{storage}. See \ref{sec:component-storage} Component Storage for further details.
...
\begin{figure}[h]
    ...
    \caption[Node States]{A UML State Diagram describing the \glspl{state} of a \gls{node}}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Learning about other \glspl{node}
...
\end{figure}
\begin{itemize}
    \item The \gls{administrator} is the only Actor interacting with the \gls{management}. He/She sends instructions (e.g. add new node) and is notified by the \gls{management} about any anomalies.
    \item \glspl{client} fetch a list of all \glspl{node} from the \gls{management} (See Component \ref{sec:component-client} \nameref{sec:component-client} for further details). As for the study project, the \gls{management} does not know anything about \glspl{client}, but this might change when authentication is implemented in the future.
    \item The \gls{management} maintains \gls{metadata} for each \gls{node} in the system and sends that information every \gls{node} when they post their \gls{metadata}.
\end{itemize}
