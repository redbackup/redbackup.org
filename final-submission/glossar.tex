...
\newglossaryentry{node-identifier}
{
    ...
    description={A unique identifier assigned to any node to distinguish it from other.}
}
...
{
    name={chunk index},
    description={A \gls{client}-side data structure that maps files on filesystem to chunks and describes their folder structures.}
}
...
{
    name={file},
    description={A document on a client system that shall be backed up. A file is represented in the system by multiple \glspl{chunk}.}
}
