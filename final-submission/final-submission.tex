...
The subsequent implementation of a architecture prototype took place in the \emph{Rust} system programming language\footnote{For more information, see \url{https://www.rust-lang.org/}}, which we learned during the course of this project. Rust enabled us to create a very stable yet efficient backup prototype.

\section*{Results}
The architecture consists of backup nodes, which store and distribute data directly over a network connection, as well as a client application that creates and restores backups to or from nodes. Lastly, a management system is introduced to allow users to manage multiple backup nodes.
...
To extend the prototype into a full functional backup system, there are multiple improvements may be implemented. Especially, the encryption and further splitting of backup data, the data distribution strategy and management application need to be extended.
...
\include{appendices/morphological_box}
