...
Today, most individuals and small to medium enterprises have limited choice with regards to data backup storage.

One possibility is the utilisation of local storage media, as e.g. hard disk drives, which requires personal effort that usually leads to missing physical backup redundancy. A second possibility is a public cloud environment, which leads to privacy issues and a high dependency on third party providers.

No easy to use, distributed backup systems with private data storage is available on the market today. \\
\vspace*{1ex}

\noindent To address this issue, we propose an architecture for a redundant distributed backup system which respects data privacy as well as data security.

The architecture consists of backup nodes which exchange data with a peer-to-peer protocol, as well as a client application that creates and restores backups. A management system is introduced to allow users to manage multiple backup nodes.
\vspace*{1ex}

\noindent Finally, we present a prototype of the proposed client and node applications with a reduced feature set, implemented in the Rust programming language.
\end{abstract}
...
\include{chapters/2_our_approach}
\include{chapters/3_discussion_and_conclusion}
...
\include{appendices/architecture}
\include{appendices/language-evaluation}
